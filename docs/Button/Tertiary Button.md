# 📌 Tertiary Button - Accessibilità

## 1. Overview del Componente
Il componente `Tertiary Button` viene utilizzato per azioni di minore importanza rispetto ai Primary e Secondary Button. Deve essere chiaramente distinguibile, accessibile tramite tastiera e compatibile con gli screen reader.
...
```html
<button type="button">Opzione avanzata</button>
```

🚫 **Esempio Errato (div non accessibile)**
```html
<div onclick="executeAction()">Opzione avanzata</div> <!-- Non accessibile -->
```
